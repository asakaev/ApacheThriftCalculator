...
var Thrift = {
    ...
    PERSISTENCE_KEY: 'lh-persistence',
    LOCAL_STORAGE: window['localStorage'] !== undefined
};
 ...
 * @readonly
 * @return {Thrift.TApplicationExceptionType} exception code
 */
 ...
 * @param {boolean=} [opt_withCredentials] - withCredentials.
 ...
 */
Thrift.Transport = Thrift.TXHRTransport = function(url, opt_withCredentials) {
    this.url = url;
    this.wpos = 0;
    this.rpos = 0;
    this.send_buf = '';
    this.recv_buf = '';
    this.withCredentials = opt_withCredentials || false;
    this.requestHeaders = {};
};
...
Thrift.TXHRTransport.prototype = {
  ...
  addHeader: function(name, value) {
     ...
     * @return {object} the browser XHR interface object
     */
     ...
     * @return {undefined|string} Nothing or the current send buffer.
    ...
    flush: function(async, callback) {
              ...
              (function() {
                ...
                return function() {
                  if (this.responseText !== '') {
                    xreq = self.getXmlHttpRequestObject();
                    xreq.open('POST', this.responseText, !!async);
                    xreq.withCredentials = self.withCredentials;

                    var keys = Object.keys(self.requestHeaders);
                    for (var i = 0; i < keys.length; i++) {
                      xreq.setRequestHeader(keys[i], self.requestHeaders[keys[i]]);
                    }
                    xreq.onload = function() {
                      self.setRecvBuffer(xreq.responseText);
                      clientCallback();
                    };
                    xreq.onprogress = function() {};

                    xreq.send(self.send_buf);
                  } else {
                    self.setRecvBuffer(xreq.responseText);
                    clientCallback();
                  }
        ...
        xreq.open('POST', this.url + '?' + Math.random(), !!async);
	      xreq.withCredentials = this.withCredentials;

        var keys = Object.keys(this.requestHeaders);
        for (var i = 0; i < keys.length; i++) {
          xreq.setRequestHeader(keys[i], this.requestHeaders[keys[i]]);
        }
     ...
     * @return {string} Characters sent by the server.
     */
    read: function(len) {
    ...
    __isOldIe: function() {
      ...
      if (myNav.indexOf('msie') != -1) {
        result = parseInt(myNav.split('msie')[1]) <= 9;
      }
      return result;
    },
     ...
     */
    __saveBuffer: function(key, value, destination) {
      if (Thrift.LOCAL_STORAGE) {
        var buffers = this.__getBuffers();
        buffers[key] = [destination, value];
        localStorage.setItem(Thrift.PERSISTENCE_KEY, JSON.stringify(buffers));
      }
    },
     ...
     * @return {!Object}
     */
    __getBuffers: function() {
      var result = {};
      if (Thrift.LOCAL_STORAGE) {
        try {
          var obj = JSON.parse(localStorage.getItem(Thrift.PERSISTENCE_KEY));
...
Thrift.TWebSocketTransport.prototype = {
     ...
     * @return {undefined|string} Nothing (undefined)
     */
    flush: function(async, callback) {
      var self = this;
      if (this.isOpen()) {
...
Thrift.Protocol.prototype = {
     ...
     * @readonly
     * @return {Thrift.Transport} The underlying transport.
     */
     ...
     * @return {AnonReadMessageBeginReturn}
     */
     ...
     * @return {object} - An object with an empty string fname property
     */
     ...
     * @return {AnonReadFieldBeginReturn}
     */
